...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(partitioningBy(a -> Sex.MALE.equals(a.getSex())));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(groupingBy(a -> a.getEmail().substring(a.getEmail().indexOf('@') + 1)));
    }
     ...
     *
     * @param emailDomain domain of the email
     * @return true if there is an account that has an email with provided domain
     */
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .map(Account::getEmail)
                .anyMatch(e -> e.endsWith(emailDomain));
    }

    /**
     * Returns account balance by its email. Throws {@link com.bobocode.fp.exception.EntityNotFoundException} with message
     ...
     * case, in all {@link Account#getFirstName()} and {@link Account#getLastName()} that are equal or longer than nameLengthBound.
