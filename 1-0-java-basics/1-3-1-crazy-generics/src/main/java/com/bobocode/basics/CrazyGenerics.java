...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
         ...
         *
         * @param list provided collection to be printed
         */
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            ...
            while (iterator.hasNext()) {
                ...
                max = isNull(max) ? next : comparator.compare(next, max) > 0 ? next : max;
            }
        ...
        public static <T> void swapElements(List<T> elements, int i, int j) {
            final var elementI = elements.get(i);
            final var elementJ = elements.get(j);
            elements.remove(i);
            elements.remove(j);
            elements.add(i, elementJ);
            elements.add(j, elementI);
        }
    }
}
